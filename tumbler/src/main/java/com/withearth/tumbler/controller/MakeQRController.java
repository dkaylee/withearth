...
public class MakeQRController {
	...
	final String url ="http://192.168.219.105:8080/tumbler/qr/tumLogin";

   /*
    * @RequestMapping("/test") public String test(HttpServletRequest req) {
    * 
    * String cafe_idx = req.getParameter("cafe_idx");
    * return "1"; }
    */
   @RequestMapping("/qr/{cafe_idx}") 
   public String makeqr(HttpServletRequest request, Model model,
                      HttpSession session,
...
//      int point = 100;
      ...
      if(!file.exists()) { 
       ...
       File temp =  new File(savePath +cafeIdx+".png");  
       ...
       //db에 데이터  저장
      //model.addAttribute("cafeIdx",cafeservice.getcafe(request,response));
     

      
       //리턴은 사용자가 원하는 값을 리턴한다. 
       //작성자는 QRCode 파일의 이름을 넘겨주고 싶었음.
      return makeqrSerivce.getmakeqr(cafe_idx); 
