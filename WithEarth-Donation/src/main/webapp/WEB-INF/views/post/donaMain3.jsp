	...
	<a href="<c:url value="/main/post/upload"/>"><input type="button" class="pBtn" value="글쓰기"></a>	
   	...
   	<script>
		...
		$(document).ready(function() {
				...
				$.ajax({
					url : "http://localhost:8080/dona/rest/user/post/list?p=1",
					type : 'GET',
					success : function(data) {
						
						console.log(data);
						$.each(data, function(index,item) {
							console.log(item.PostFile);
							
							var html ='<div class="article" onClick="location.href=\'<c:url value="/main/post/detail?idx='+item.donaIdx+'"/>\'">';
							html += '<div class="article-img">';
							html += '<img alt="thumbnail" class="img" src="<c:url value="/fileupload/post/s_'+item.files+'"/>">';	
							html += '</div>';
